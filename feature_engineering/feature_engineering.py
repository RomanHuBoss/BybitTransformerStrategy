...
class FeatureEngineer:
    def __init__(self, fit: bool = False):
    ...
    def __adjust(self, base_window):
        return max(2, int(base_window * CFG.assets.time_frame / 30))

    def generate_from_raw_csv(self, fit: bool = True, use_logging: bool = True):
        df = pd.read_csv(CFG.paths.raw_csv)
        return self.__generate_features(df, fit, use_logging)

    def generate_from_df(self, df: pd.DataFrame, fit: bool = True, use_logging: bool = True):
        return self.__generate_features(df, fit, use_logging)
    ...
    def __generate_features(self, df: pd.DataFrame, fit: bool = True, use_logging: bool = True) -> pd.DataFrame:
        ...
        features['returns_mean'] = returns.rolling(self.__adjust(10)).mean().shift(shift)
        features['returns_std'] = returns.rolling(self.__adjust(10)).std().shift(shift)
        features['returns_skew'] = returns.rolling(self.__adjust(20)).apply(lambda x: skew(x)).shift(shift)
        features['returns_kurtosis'] = returns.rolling(self.__adjust(20)).apply(lambda x: kurtosis(x)).shift(shift)
        features['range_pct'] = (df['high'] - df['low']) / df['close'].shift(1)

        # ATR (short, mid, long)
        for label, w in [('short', 3), ('mid', 8), ('long', 20)]:
            window = self.__adjust(w)
        ...
        # Breakout detectors
        breakout_window = self.__adjust(20)
        ...
        short_window = self.__adjust(5)
        long_window = self.__adjust(20)
        ...
        # EMA Cross Momentum
        ema_5 = df['close'].ewm(span=self.__adjust(5), min_periods=self.__adjust(5)).mean()
        ema_10 = df['close'].ewm(span=self.__adjust(10), min_periods=self.__adjust(10)).mean()
        features['ema_cross_momentum'] = (ema_5 - ema_10).shift(shift)

        # Micro ATR
        micro_window = self.__adjust(5)
        ...
        rolling_mean_window = self.__adjust(20)
    ...
    def forward(self, X: pd.DataFrame, window_size: int = CFG.assets.window_size) -> np.ndarray:
        data = []
